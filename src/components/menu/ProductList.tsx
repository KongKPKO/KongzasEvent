               ...
               <div key={product.id} className={`bg-white rounded-xl shadow-sm overflow-hidden flex flex-col h-full border border-gray-100 transition-all ${qty > 0 ? 'ring-2 ring-pink-500' : ''}`}>
                  ...
                  <div className="p-2.5 flex flex-col flex-1 justify-between">
                     ...
                     <div className="flex flex-col gap-1.5">
                        ...
                        {qty === 0 ? (
                           <button onClick={() => product.status !== 'soldout' && onUpdateQuantity(product.id, 1)} disabled={product.status === 'soldout' || isOrderSent} className={`w-full rounded-md py-1 flex items-center justify-center gap-1 text-[10px] font-bold transition-all ${product.status === 'soldout' || isOrderSent ? 'bg-gray-100 text-gray-400' : 'bg-gray-900 text-white active:scale-95'}`}><ShoppingBag size={10} /> ADD</button>
                        ) : (
                           <div className="flex items-center justify-between bg-pink-50 rounded-md p-0.5 border border-pink-100">
                              <button onClick={() => onUpdateQuantity(product.id, -1)} className="w-6 h-6 rounded bg-white text-pink-600 flex items-center justify-center shadow-sm"><Minus size={12} /></button>
                              <span className="font-bold text-xs">{qty}</span>
                              <button onClick={() => onUpdateQuantity(product.id, 1)} className="w-6 h-6 rounded bg-pink-500 text-white flex items-center justify-center shadow-md"><Plus size={12} /></button>
                           </div>
                        )}
                     </div>
                  </div>
