-- Phase 2 post-event shipping orders.
-- This migration is intentionally written only; do not apply from Codex.
...
alter table public.orders
  ...
  check (pickup_status in ('not_required', 'awaiting_pickup', 'picked_up', 'cancelled', 'expired', 'awaiting_shipment', 'shipped'));

drop function if exists public.create_preorder_with_stock(uuid, jsonb, text, text, text, uuid, text, text, text);
...
begin
  ...
  if v_event.status not in ('Confirmed', 'confirmed')
     and not (coalesce(v_event.selling_mode, 'live') = 'post_event' and v_event.status = 'Ended') then
  ...
  if coalesce(v_event.selling_mode, 'live') not in ('preorder', 'post_event') then
    raise exception 'preorder_not_open';
  end if;

  if coalesce(v_event.selling_mode, 'live') = 'post_event' then
  ...
  if coalesce(v_event.selling_mode, 'live') <> 'post_event' and v_event.end_date < now() then
  ...
  values (
    ...
    null,
    p_client_request_id,
    case when coalesce(v_event.selling_mode, 'live') = 'post_event' then 'post_event' else 'preorder' end,
    v_pickup_code,
...
begin
  ...
  if v_order.order_type not in ('preorder', 'post_event') then
  ...
  v_next_pickup_status := case when v_order.order_type = 'post_event' then 'awaiting_shipment' else 'awaiting_pickup' end;
  ...
  return query select v_order.id, 'payment_confirmed'::text, v_next_pickup_status, v_now;
end;
$$;
...
begin
  ...
  for v_order in
    ...
    where o.event_id = p_event_id
      ...
      and o.pickup_status in ('awaiting_pickup', 'awaiting_shipment')
...
drop function if exists public.list_preorder_payment_review(uuid, text);
...
-- Customers must be able to see an ended event while its post-event store is
-- open; otherwise the whole post-order flow is invisible to anon users.
-- Note: events that have finished are marked status='Ended', so the post-event
-- branch accepts Confirmed or Ended.
...
create policy events_public_read on public.events
  ...
  using (
    (
      ...
      and (
        (status = any (array['Confirmed'::text, 'Cancelled'::text]) and end_date >= now())
        or (
          selling_mode = 'post_event'
          and status = any (array['Confirmed'::text, 'Ended'::text])
          and (preorder_closes_at is null or preorder_closes_at >= now())
        )
      )
    )
