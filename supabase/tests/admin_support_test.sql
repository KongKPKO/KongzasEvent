...
reset role;
set local role anon;
select throws_ok($$select public.admin_order_email_claim('a3000000-0000-4000-8000-000000000001',gen_random_uuid(),'Support reason')$$,'42501',null,'anonymous resend denied');
reset role;
set local role authenticated;
select set_config('request.jwt.claims','{"sub":"a1000000-0000-4000-8000-000000000002","email":"support-owner@test.local"}',true);
select throws_ok($$select public.admin_order_email_claim('a3000000-0000-4000-8000-000000000001',gen_random_uuid(),'Support reason')$$,'P0001','forbidden','merchant cannot resend as admin');
select throws_ok($$select public.admin_support_history('a3000000-0000-4000-8000-000000000001','Support reason')$$,'P0001','forbidden','merchant cannot read support history');
select set_config('request.jwt.claims','{"sub":"a1000000-0000-4000-8000-000000000001","email":"support-admin@test.local"}',true);
select throws_ok($$select public.admin_order_email_claim('a3000000-0000-4000-8000-000000000001',gen_random_uuid(),'x')$$,'P0001','invalid_request','resend reason required');
select throws_ok($$select public.admin_order_email_claim('a3000000-0000-4000-8000-000000000002',gen_random_uuid(),'Support reason')$$,'P0001','unsupported_order','offline excluded');
select throws_ok($$select public.admin_order_email_claim('a3000000-0000-4000-8000-000000000003',gen_random_uuid(),'Support reason')$$,'P0001','missing_email_or_code','missing email excluded');
select is(public.admin_order_email_claim('a3000000-0000-4000-8000-000000000001','a8000000-0000-4000-8000-000000000001','Support reason')->>'send','true','first send claimed');
select is(public.admin_order_email_claim('a3000000-0000-4000-8000-000000000001','a8000000-0000-4000-8000-000000000001','Support reason')->>'send','false','same request never sends twice');
select throws_ok($$select public.admin_order_email_claim('a3000000-0000-4000-8000-000000000001','a8000000-0000-4000-8000-000000000001','Changed reason')$$,'P0001','request_conflict','request cannot be reused for different intent');
select throws_ok($$select public.admin_order_email_claim('a3000000-0000-4000-8000-000000000001',gen_random_uuid(),'Support reason')$$,'P0001','cooldown','cooldown applies across requests');
select throws_ok($$select * from public.admin_order_email_resends$$,'42501',null,'attempt ledger not directly readable');
select throws_ok($$update public.admin_order_email_resends set status='accepted'$$,'42501',null,'client cannot forge outcome');
select ok(not(public.admin_support_history('a3000000-0000-4000-8000-000000000001','Support reason')::text like '%SECRET%'),'history excludes secrets');
reset role;
update public.admin_order_email_resends set status='accepted',created_at=now()-interval '2 minutes';
set local role authenticated;
select is(public.admin_order_email_claim('a3000000-0000-4000-8000-000000000001','a8000000-0000-4000-8000-000000000002','Support reason')->>'send','true','explicit resend allowed after success');
reset role;
select is((select count(*) from public.admin_order_email_resends where order_id='a3000000-0000-4000-8000-000000000001'),2::bigint,'original attempt retained');
select ok(not exists(select * from public.orders except select * from before_orders),'resend does not change orders');
select ok(not exists(select * from public.order_payments except select * from before_payments),'resend does not change money');
select ok(not exists(select * from public.products except select * from before_products),'resend does not change stock');
select * from finish();
rollback;
